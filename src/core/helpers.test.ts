...
describe("schemaTypeToCliType", () => {
	...
	it("uses the non-null schema from nullable anyOf/oneOf unions", () => {
		expect(
			schemaTypeToCliType({ anyOf: [{ type: "integer" }, { type: "null" }] }),
		).toEqual({
			type: "integer",
			suffix: "",
		});
		expect(
			schemaTypeToCliType({ oneOf: [{ type: "object" }, { type: "null" }] }),
		).toEqual({
			type: "string",
			suffix: " (JSON object)",
		});
...
describe("coerceValue", () => {
	...
	it("coerces arrays from JSON, comma strings, and single values", () => {
		...
		expect(
			coerceValue("1,2", { type: "array", items: { type: "integer" } }),
		).toEqual([1, 2]);
		expect(
			coerceValue("true,false", { type: "array", items: { type: "boolean" } }),
		).toEqual([true, false]);
	});
	...
	it("coerces nullable anyOf/oneOf values using the non-null schema", () => {
		expect(
			coerceValue("2", { anyOf: [{ type: "integer" }, { type: "null" }] }),
		).toBe(2);
		expect(
			coerceValue('{"a":1}', { oneOf: [{ type: "object" }, { type: "null" }] }),
		).toEqual({
			a: 1,
		});
	});
...
describe("coerceAndValidateValue", () => {
	it("validates coerced values with JSON Schema", () => {
		expect(
			coerceAndValidateValue("2", { type: "integer", enum: [1, 2] }, "--count"),
		).toBe(2);
		expect(() =>
			coerceAndValidateValue("bad", { type: "integer" }, "--count"),
		).toThrow(/--count failed validation/);
	});
});
...
describe("filterCommands", () => {
	...
	it("filters by methods while preserving MCP commands", () => {
		expect(
			filterCommands(commands, { methods: ["GET"] }).map((c) => c.name),
		).toEqual(["list-pets", "echo"]);
	});
